...
VkResult generateMipmaps(VkImage image, VkFormat imageFormat, uint32_t texWidth,
                         uint32_t texHeight, uint32_t mipLevels,
                         VkQueue graphicsQueue, VkCommandPool pool,
...
VkResult createTextureImage(VkImage *textureImage,
  ...
  *mipLevels =
      (uint32_t)log2((texWidth - texHeight > 0) ? texWidth : texHeight) + 1;
  ...
  VkResult res = createBuffer(&stagingBuffer, &stagingBufferMemory, imageSize,
  ...
  res = createImage(
      textureImage, textureImageMemory, texWidth, texHeight, *mipLevels,
  ...
  res = transitionImageLayout(*textureImage, VK_FORMAT_R8G8B8A8_SRGB,
                              VK_IMAGE_LAYOUT_UNDEFINED,
                              VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, *mipLevels,
                              device, pool, graphicsQueue);
  CHECK(res, "failed to transition image layout to transfer\n");

  res = copyBufferToImage(*textureImage, stagingBuffer, texWidth, texHeight,
                          device, pool, graphicsQueue);
  CHECK(res, "failed to copy buffer to image\n");

  // res = transitionImageLayout(*textureImage,VK_FORMAT_R8G8B8A8_SRGB,
  // VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, *mipLevels,
  // VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, device, pool, graphicsQueue);
  res = generateMipmaps(*textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth,
                        texHeight, *mipLevels, graphicsQueue, pool,
...
VkResult createTextureImageView(VkImageView *textureImageView,
                                uint32_t mipLevels, VkImage textureImage,
                                VkDevice device) {
  VkResult res = createImageView(textureImageView, mipLevels, textureImage,
                                 VK_IMAGE_ASPECT_COLOR_BIT,
                                 VK_FORMAT_R8G8B8A8_SRGB, device);
  CHECK(res, "failed to create image view\n");

  return VK_SUCCESS;
}

VkResult createTextureSampler(VkSampler *textureSampler, VkDevice device,
                              VkPhysicalDevice physicalDevice) {
